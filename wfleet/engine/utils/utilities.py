...
def get_hash_bucket_number(value: str, buckets: int) -> int:
    ...
    if buckets <= 0:  # if buckets number <= 0 - generated bucket number is always 0
        log.debug(
            f"get_hash_bucket_number(): buckets number [{buckets}] <= 0, return 0!"
        )
        return 0
    ...
    hex_hash = hashlib.md5(
        value.encode("utf-8")
    ).hexdigest()  # generate hexadecimal hash
...
def add_value_to_hashmap(hashmap: dict, value: str, buckets: int) -> dict:
    ...
    """
    log.debug(
        f"add_value_to_hashmap(): hashmap [{hashmap}], value [{value}], buckets [{buckets}]."
    )

    if hashmap is None or not isinstance(
        hashmap, dict
    ):  # fail-fast - hash map type check
    ...
    bucket_number = get_hash_bucket_number(
        value, buckets
    )  # bucket number for the value
    ...
    for letter1 in RUS_CHARS + ENG_CHARS + NUM_CHARS:
        for letter2 in RUS_CHARS + ENG_CHARS + NUM_CHARS:
            result = add_value_to_hashmap(
                result, letter1 + letter2, buckets
...
def generate_timed_filename(postfix: str) -> str:
    ...
    """
    log.debug(
        f"generate_timed_filename(): generating file name with postfix {postfix}."
    )

    result: str = datetime.now().strftime(
        const.SCRAPER_CACHE_DIRECTORY_TIMESTAMP_PATTERN
    )
